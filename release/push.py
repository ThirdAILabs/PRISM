...
from cloud_provider_interface import CloudProvider
from docker_constants import images_to_build, images_to_pull_from_private
from utils import Credentials, image_name_for_branch, load_config, get_root_absolute_path
...
def build_images(
    ...
    for image in images_to_build:
        ...
        # For env vars that should be hardcoded at image build time, include them in buildargs
...
def verify_tag(
    provider: CloudProvider, image_ids: Dict[str, str], tag: str, branch: str
) -> None:
    """
...
def main() -> None:
    ...
    if provider_name == "azure":
        if not config["azure"]["registry"]:
            config["azure"]["registry"] = "prismresearchsecurity.azurecr.io"
        ...
        if not pull_username or not pull_password:
            new_pull_credentials = provider.create_credentials(
                name=f"prism-pull{sanitized_branch}",
                image_names=[
                    image_name_for_branch(image.name, args.branch)
                    for image in images_to_build
                ]
        ...
        image_ids = build_images(
            provider, args.branch, tag, args.no_cache
        )
