...
Defines the `CloudProvider` class, an abstract base class that outlines the methods required for a cloud provider interface.
...
provider: azure
azure:
  registry: prismresearchsecurity.azurecr.io
  branches:
    prod:
      version: "1.0.0"
      push_credentials:
...
1. **Implement the Provider Class**: Create a new provider class that implements the `CloudProvider`. This class should handle the specific interactions with the new cloud provider.
