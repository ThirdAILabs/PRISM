...
func loadMultihopEntities() []string {
	...
	utils.ParseJsonData("../../data/docs_and_press_releases.json", &countryToArticles)
	...
	var articles []record
	utils.ParseJsonData("../../data/auxiliary_webpages.json", &articles)
...
func loadWatchlistEntities() []string {
	...
	utils.ParseJsonData("../../prism/reports/flaggers/eoc/data/alias_to_source.json", &aliasToSource)
...
func generateData(queries []string, outputFile string) {
	...
	samples := make(chan utils.Sample, len(queries))
	...
	for i := 0; i < len(queries); i += 100 {
		...
		go func(batch []string) {
			...
			for _, query := range batch {
				...
				samples <- utils.Sample{Entity: query, Queries: res}
			}
	...
	samplesList := make([]utils.Sample, 0, len(queries))
	for s := range samples {
