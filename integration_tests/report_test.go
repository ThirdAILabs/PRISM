	...
	for i, reportId := range reportIds {
		...
		for flagType, expectedCount := range expectedFlags {
			if len(report.Content[flagType]) >= expectedCount-2 && len(report.Content[flagType]) <= expectedCount+2 {
				t.Fatalf("Report %s (%s): expected [%d, %d] flags of type %s, got %d", reportRequests[i].AuthorName, reportRequests[i].AuthorId, expectedCount-2, expectedCount+2, flagType, len(report.Content[flagType]))
			}
		}
	}
}
