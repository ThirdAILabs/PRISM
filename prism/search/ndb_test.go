...
func checkQuery(t *testing.T, ndb search.NeuralDB, query string, constraints search.Constraints, expectedIds []uint64) {
...
func TestBasicRetrieval(t *testing.T) {
	db, err := search.NewNeuralDB(t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	checkQuery(t, db, "a & b c", search.Constraints{"q1": search.EqualTo(true)}, []uint64{0, 7, 11})

	checkQuery(t, db, "f g", nil, []uint64{6, 10, 0, 5, 7})

	checkQuery(t, db, "f g", search.Constraints{"q2": search.EqualTo(true)}, []uint64{6, 5, 7})
}
...
func TestConstrainedSearch(t *testing.T) {
	...
	constraints := search.Constraints{
		"k1": search.GreaterThan(3.0),
		"k2": search.EqualTo(false),
		"k3": search.EqualTo(7),
		"k4": search.LessThan("peach"),
	}
...
func TestReturnsCorrectChunkData(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		constrainedResults, err := db.Query(query, 5, search.Constraints{"type": search.EqualTo("second")})
		if err != nil {
			t.Fatal(err)
		}
...
func TestFinetuning(t *testing.T) {
	...
	constraints := search.Constraints{"key": search.EqualTo(true)}
...
func TestDocVersioning(t *testing.T) {
	...
	getSources := func() []search.Source {
		sources, err := db.Sources()
		if err != nil {
			t.Fatal(err)
		}

		slices.SortFunc(sources, func(a, b search.Source) int {
