...
using thirdai::automl::udt::Flash;
...
unsigned int StringList_len(StringList_t *list) { return list->list.size(); }
const char *StringList_at(StringList_t *list, unsigned int i) {
  return list->list.at(i).c_str();
}
...
struct Flash_t {
  ...
  Flash_t() : flash(Flash::make("entity", "entity", "large")) {}
};

Flash_t *Flash_new(const char **err_ptr) {
  try {
    return new Flash_t();
  } catch (const std::exception &e) {
    copyError(e, err_ptr);
...
void Flash_train(Flash_t *flash, const char *filename, const char **err_ptr) {
  try {
    flash->flash->trainOnFile(filename);
  } catch (const std::exception &e) {
    copyError(e, err_ptr);
    return;
  }
...
StringList_t *Flash_predict(Flash_t *flash, const char *query,
                            unsigned int top_k, const char **err_ptr) {
  try {
    auto results = flash->flash->predictSimple(query, top_k);
...
void Flash_free(Flash_t *flash) { delete flash; }
