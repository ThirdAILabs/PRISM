...
// #cgo linux LDFLAGS: -L./lib/linux_x64 -L./lib/linux_arm64 -lthirdai -lrocksdb -lutf8proc -lcryptopp -fopenmp -lssl -lcrypto
...
func NewNeuralDB(savePath string) (NeuralDB, error) {
...
func NewFlash() (Flash, error) {
	var err *C.char
	flash := C.Flash_new(&err)
	if err != nil {
		defer C.free(unsafe.Pointer(err))
		return Flash{}, errors.New(C.GoString(err))
	}

	return Flash{flash: flash}, nil
}

func (flash *Flash) Free() {
	C.Flash_free(flash.flash)
}

func (flash *Flash) Train(filename string) error {
	filenameCStr := C.CString(filename)
	defer C.free(unsafe.Pointer(filenameCStr))

	var err *C.char
	C.Flash_train(flash.flash, filenameCStr, &err)
	if err != nil {
		defer C.free(unsafe.Pointer(err))
		return errors.New(C.GoString(err))
	}
...
func (flash *Flash) Predict(query string, topk int) ([]string, error) {
	if topk <= 0 {
		return nil, errors.New("topk must be > 0")
	}
	...
	var err *C.char
	results := C.Flash_predict(flash.flash, queryCStr, C.uint(topk), &err)
	if err != nil {
		defer C.free(unsafe.Pointer(err))
		return nil, errors.New(C.GoString(err))
	}

	nResults := C.StringList_len(results)
	output := make([]string, 0, nResults)

	for i := C.uint(0); i < nResults; i++ {
		output = append(output, C.GoString(C.StringList_at(results, i)))
	}
