        ...
        Object.keys(reportContent[flagType]).forEach((flag, index2) => {
            if (reportContent[flagType][index2].hasOwnProperty('Connections')) {
                ...
                if (reportContent[flagType][index2].Connections !== null) {
                    ...
                    if (reportContent[flagType][index2].hasOwnProperty('EntityMentioned') && !reportContent[flagType][index2].FrequentCoauthor) {
...
function generateGraphData(data) {
    ...
    */
    ...
    for (let level = 0; level <= maxLevel; level++) {
        const maxConnections = Math.max(...levelToNodes[level].map(nodeId => nodeToNumConnections[nodeId] || 0));
