    ...
    return (
        <div className={`side-panel ${isOpen ? 'open' : ''}`}>
            <div className="panel-content">
                {/* Header */}
                <div className="side-panel-header" style={{ display: 'flex', alignItems: 'center', padding: '10px 20px', paddingTop: '0', borderBottom: '1px solid #e0e0e0', gap: '20px' }}>
                    <img src={PRISM_LOGO} alt="PRISM" style={{ width: "150px", height: '30px', marginLeft: '10%' }} />
                </div>
                {/* Navigation */}
                <nav className="navigation">
                    <ul className="nav-list">
                        <li className={`nav-item ${currentLocation === '/' ? 'active' : ''}`} onClick={handleIndividualClick}>
                            <span className="nav-icon"><FaRegUserCircle /></span>
                            <span className="nav-text">Individual Assessment</span>
                        </li>
                        <li className={`nav-item ${currentLocation === '/university-assessment' ? 'active' : ''}`} onClick={handleUniversityClick}>
                            <span className="nav-icon"><FaUniversity /></span>
                            <span className="nav-text">University Assessment</span>
                        </li>
                        <li className={`nav-item ${currentLocation === '/entity-lookup' ? 'active' : ''}`} onClick={handleEntityClick}>
                ...
                <div className="reports">
                    ...
                    {reports.map((report, index) => (
                        (index < 10) && <div key={report.Id} className="report-item" onClick={handleReportClick.bind(null, report)}>
