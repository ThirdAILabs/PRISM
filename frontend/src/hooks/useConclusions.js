...
export function useConclusions(author, flags, threshold) {
  ...
  const summary = useMemo(() => {
    ...
    if (numUniqueInstitutions) {
      messages.push(
        `${numUniqueInstitutions} unique institutions of concern are listed across ${author}'s ${papers(
          numUniqueInstitutions
        )}.`
      );
    }
    ...
    if (numUniqueFunders) {
      messages.push(
        `${numUniqueFunders} unique funders of concern are listed across ${author}'s ${papers(
          numUniqueFunders
        )}.`
      );
    }
    if (foreignFundingFrequency) {
      messages.push(
        `${foreignFundingFrequency} ${papers(
          foreignFundingFrequency
        )} received funding from foreign sources.`
      );
    }
    Object.entries(institutionFrequencies)
      .sort(([_inst1, freq1], [_inst2, freq2]) => freq2 - freq1)
      .slice(0, 3)
      .forEach(([inst, freq]) => {
        messages.push(`${inst} appears as an affiliation in ${freq} ${papers(freq)}.`);
      });
    Object.entries(funderFrequencies)
      .sort(([_inst1, freq1], [_inst2, freq2]) => freq2 - freq1)
      .slice(0, 3)
      .forEach(([inst, freq]) => {
