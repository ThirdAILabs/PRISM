...
export function useVisualizationData(
  ...
  const flagByType = useMemo(() => {
    ...
    function fromList(list, flag) {
      return flag.Entities.flatMap((entity) => entity['lists']).reduce(
        (prev, curr) => list.includes(curr) || prev,
        false
      );
    }
  ...
  const sortedAffiliations = Object.keys(affiliations).toSorted(
    (a, b) => affiliations[b][0] - affiliations[a][0]
  );
  const showAffiliations = Object.fromEntries(
    sortedAffiliations.map((affil) => [affil, affiliations[affil][1]])
  );
  ...
  const data = {
    name: name,
    total_score: Object.keys(flagByType)
      .map((flagType) => weights[flagType] * flagByType[flagType].length)
      ...
      >
        <div style={{ maxHeight: '500px', overflowX: 'scroll' }}>
          {sortedAffiliations.map((affiliation, index) => (
            ...
            >
              <input
                key={index}
                style={{ height: '12px', width: '12px', marginRight: '3px', marginTop: '5px' }}
                type="checkbox"
                checked={affiliations[affiliation][1]}
                onClick={() => {
                  setAffiliations((prev) => ({
                    ...prev,
                    [affiliation]: [prev[affiliation][0], !prev[affiliation][1]],
                  }));
                }}
              />
              <text style={{ textAlign: 'left', marginLeft: '10px' }}>{affiliation}</text>
            </div>
          ))}
        </div>
        <button
          ...
          onClick={() => {
            setAffiliations((prev) =>
              Object.fromEntries(
                Object.keys(affiliations).map((affil) => [affil, [affiliations[affil][0], false]])
              )
            );
          }}
        >
        ...
        <button
          ...
          onClick={() => {
            setAffiliations((prev) =>
              Object.fromEntries(
                Object.keys(affiliations).map((affil) => [affil, [affiliations[affil][0], true]])
              )
            );
          }}
        >
