# Instructions to set up PRISM on Blade Server

**Note: Running PRISM on Blade will require forwarding the ports for the frontend, backend, and Keycloak.**
...
```bash
cd PRISM/prism/cmd/worker
```
3. Make a copy of `cmd/worker/config_tmp.yaml` and fill in the fields.
4. Start the worker: 
```bash
...
```
2. Navigate to ```/PRISM/frontend/src/api/constants.js``` and update ```prismBaseUrl``` to ```http://localhost:8082```  or whatever you backend endpoint is. 
